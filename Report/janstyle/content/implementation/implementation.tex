...
The \code{Type} class is the starting point for users and developers of the library. It provides 4 purposes:
...
Section ABC discusses how Type's classes interact to provide its rich-text functionality. One responsibility of the \code{Type} base class is to instanitate all other classes needed for its API and provide getters to each instance, so that each instance can access the other instantiated classes.
...
We've learned how to extend. For the sake of software architecture we don't add extensions to the api in various modules where we please but have many modules implementing functinality and one module setting tha api making use of the other classes. the other classes are written in a way that the api module usually only requires one or 2 function calls (as few as possible)
