...
The namespace provides a structure for the classes of the library and prevents the pollution of the global namespace and possible name conflicts. In terms of structure, this does not only encapsulate classes related to the library but also allows nesting. This way sub-namespaces can be created, which is especially important for other developers extending the library. The \code{Type} base class already creates a sub-namespace \code{Type.Events} for events.

Section ABC discusses how Type's classes interact to provide its rich-text functionality. One responsibility of the \code{Type} base class is to instanitate all other classes and provide getters to each instance, so that each instance can access the other instantiated classes.
