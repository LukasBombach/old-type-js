...
The \code{EventApi} class provides an API with three methods
...
Using the \code{Oop} class (see \refsection{sec:oop_class}), these methods will be inherited by a \code{Type} instance. This way, using the \code{trigger} method, events can be triggered and be listened to with the \code{on} method within a type instance. 
and by the \code{Type} class itself The latter ist used for triggering the ''ready'' event on the 

\subsection{Input Event}
