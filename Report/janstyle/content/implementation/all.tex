...
An approach to create a structure for the library and achieve strong decoupling would be to follow the principles of modular programming. jQuery and CKEditor demonstrate a software architecture in which a core object mainly provides a system to extend its functionality, but does not offer many methods itself\footnote{CKEditor provides a framework for implementing components for it, but does not offer any rich-text functionality in its core. jQuery provides low-level utility methods for JavaScript apart from methods to extend it.}. The actual functionality of both libraries is implemented through plugins while the libraries are usually bundled with a set of ''core plugins'' that provide basic features.
