...
The \code{OOP} class extends the contructor pattern with basic classical inheritance. It provides the method \code{inherits} that will duplicate and copy the \code{prototype} from one \code{Function} object to another. It also copy attributes and methods defined on the \code{Function} object itself to implement inheritance of static definitions and adds the attribute \code{\_super} to the inheriting \code{Function} object to referencing its parent class to enable child classes to access their respective superclasses.
