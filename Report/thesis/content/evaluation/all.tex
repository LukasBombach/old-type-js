...
The library of this thesis implements basic features or rich-text editing. Users can type inside an editor, format text, place and move a caret, draw a selection and copy and paste text. Many of the intended features,  have not been implemented. These features include:

\begin{enumerate}
\item Behavior of the enter key in lists and headlines.
\item Contents pasted from the clipboard are not being sanitized.
\item Lists cannot be added.
\item Images cannot be added.
...
\item Lists can already be inserted, but they cannot be removed 
\item Images cannot be added.
\item Formatting does not allow for style and class attributes.
\item Formatting commands do not distinguish between inline and block commands.
\item Real-time collaboration is unstable.
...
To implement the desired behavior, input filters must be implemented, that check 


Most of these features could not be implemented to to

The lack of these features is primarily due to the com

To enable lists, input filters must be implemented that will 
