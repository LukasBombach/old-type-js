...
The libraries available still have downsides. Most importantly, only a few of them work. As a web-developer, the best choices are either CKEditor or TinyMCE. From there on, most editors do not really work. Piotrek Koszuliński, core developer of CKEditor comments this on StackOverflow as follows:
...
However, calling commands will only manipulate the document's DOM tree, in order to format the text. This can also be achieved without the editing APIs, effectively avoiding unfixable bugs and enabling a consistend behaviour across all browsers.
