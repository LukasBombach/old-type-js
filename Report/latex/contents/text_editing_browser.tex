...
With their implementation, Google demonstrates it is possible to implement a fully featured rich-text editor using only JavaScript and not using HTML editing APIs. However, fetching input and modifying text will not suffice to implement a text editor. There is many more things, that need to be considered. 
...
When not using editing APIs, all of this must be implemented with JavaScript, to \textit{fake} an input field. To make it \textit{seem} there is an input field where there is none. And to do it in a way, that the user does not notice the difference.
