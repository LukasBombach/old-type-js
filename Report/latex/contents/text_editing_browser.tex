...
Google points out that implementing and editor using HTML editing APIs comes with the restriction that such an editor can only offer the least common denominator of functions implemented by all (supported) browsers. They argue, if one browser does not support a specific feature or its implementation is buggy, it cannot be supported by the editor\footnote{\url{http://googledrive.blogspot.fr/2010/05/whats-different-about-new-google-docs.html}, last checked on 07/18/2015}. This is mostly true, although it is to be noted, that editors like CKEditor show, that some bugs can be worked around as well as some functionality be added through JavaScript. These workaround still have limitations and not everything can be fixed. In particular, the editing mode might not be able to handle content inserted or altered by workarounds.
...
Manipulating the DOM, e.g. changing the contents of a website---or particular parts of a website---has been possible since the first implementations of JavaScript and JScript and has been standardised in 1998 with the W3C's ''Document Object Model (DOM) Level 1 Specification'' specification\footnote{\url{http://www.w3.org/TR/REC-DOM-Level-1/level-one-core.html}, last checked on 07/10/2015}. Any DOM manipulation that can be achieved with HTML editing APIs can also be achieved using this API.

To implement formatting commands, the currently selected text can be wrapped in newly created tags, according to the command that has been executed. For instance, to implement the \texttt{bold} commmand of \texttt{execCommand}, the selected text can be read with the browser's selection API\footnote{\url{https://developer.mozilla.org/en-US/docs/Web/API/Selection}, last checked on 07/18/2015}\footnote{Internet Explorer prior version 9 uses a non-standard API \url{https://msdn.microsoft.com/en-us/library/ms535869(v=VS.85).aspx}, last checked on 07/18/2015} and wrapped in \texttt{strong} tags using basic DOM Level 1 methods. A specific cross-browser implementation will be discussed in section XY.

Google completely rewrote its document editor in 2010 abandoning HTML editing APIs entirely. In a blog post\footnote{\url{http://googledrive.blogspot.fr/2010/05/whats-different-about-new-google-docs.html}, last checked on 07/18/2015}, they stated some of the reasons discussed in section XYZ. Using the editing mode, if a browser has a bug in a particular function, Google won't be able to fix. In the end, they could only implement ''least common denominator of features''. Furthermore, abanding HTML editing APIs, enables features not possible before like tab stops for layouting. 

With this implementation, Google demonstrates it is possible to implement a fully featured rich-text editor using only JavaScript and not using HTML editing APIs. However, fetching input and modifying text will not suffice to implement a text editor. There is many more things, that need to be considered. 

The most obvious part is probably the text caret. Even if a user types on his or her keyboard, a caret must be seen on the screen to know where the input will be inserted. The caret also need to be responsive to the user's interaction. In particular, the user must be able to click anywhere in the editable text and use the arrow keys to move it.



All of these parts that make up for a text editor are natively implemented by browsers. Switching an element to editing mode enables these components automatically. The user can click in a text and move the caret with the keyboard's arrow keys. He or she can copy and paste text with no further scripting.
When not using editing APIs, all of this must be implemented with JavaScript, to \textit{fake} an input field. To make it \textit{seem} there is an input field where there is none. And to do it in a way, that the user does not notice.

---

for an implmentation of a text editor. There is many things that need to be considered to make it seem like an element actually is a ''native'' text input

The switching an element to editing mode, makes this element somewhat of a native text input element. Users can click on it and a text caret appear

 There is many more things that need to be considered to make it 

The basic requirement for this is fetching the user's keyboard input and actually modifying text on the website.
Apart from actually modifying the DOM, there are many things to consider, when not using HTML editing APIs. 

 This alone

Switching to editing mode does more than that though. 
 to make a particular element in a website \textit{seem} like it is a text input. 


Many of the techniques to mimick a native text input while the text will actually be read and inserted by JavaScript can be found on web-based code editors like ACE or CodeMirror.


%% Text formatting can be mimicked by finding the selected text using the browser's selection API and wrapping it in the desired 
