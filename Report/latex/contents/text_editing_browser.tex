...
\paragraph{Unspecific API}

\paragraph{Lack of control}

of output

\paragraph{No specification on output}

The specifications on the HTML editing APIs do not state what markup to generate with specific commands. There are vast differences in the implememtations of all major browsers. Calling the \texttt{italic} command, this is the output of Internet Explorer, Firefox and Chrome:
...
\noindent This is a major problem for web development, because it makes processing input very difficult. Also Firefox's output is considered a bad style while Internet Explorer's output is semantically incorrect\footnote{\url{https://developer.mozilla.org/en-US/docs/Web/HTML/Element/i#Notes}, last checked on 07/15/2015} for most use cases.

\paragraph{Differences in browser behaviour}
