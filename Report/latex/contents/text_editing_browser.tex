...
To implement formatting commands, the currently selected text can be wrapped in newly created tags, according to the desired format. For instance, to implement the \texttt{bold} commmand of \texttt{execCommand}, the selected text can be read with the browser's selection API\footnote{\url{https://developer.mozilla.org/en-US/docs/Web/API/Selection}, last checked on 07/18/2015}\footnote{Internet Explorer prior version 9 uses a non-standard API \url{https://msdn.microsoft.com/en-us/library/ms535869(v=VS.85).aspx}, last checked on 07/18/2015} and wrapped in \texttt{strong} tags using basic DOM Level 1 methods. A specific cross-browser implementation will be discussed in section XY.

Google demonstrates it is possible to implement a rich-text editor in the browser entirely written in JavaScript without requiring the HTML editing API with their implementation of the Google document editor\footnote{\url{http://googledrive.blogspot.fr/2010/05/whats-different-about-new-google-docs.html}, last checked on 07/18/2015}. Apart from actually modifying the DOM, there are many things to consider to make a particular element in a website \textit{seem} like it is a text input. 
