...
Algorithm~\ref{alg:remove_pseudocode} demonstrates a procedure to mimick the \texttt{delete} command of \texttt{execCommand}. With formatting and removing text as well as inserting elements, we can find equivalents for all commands of the HTML editing APIs related to manipulating rich-text. Aside from text or DOM manipulation, editing APIs offer commands for undo/redo or clipboard capabilites. Table \ref{tab:execCommand_equivalents} lists all commands available for \texttt{execCommand} and possible equivilents with DOM Level 1 methods. However, these are just simplified examples. A specific implementation accounting for all commands and all edge-cases will be discussed in chapters {ch:concept} and {ch:implementation}.
